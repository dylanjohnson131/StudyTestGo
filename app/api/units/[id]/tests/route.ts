...
import { addUnitTestAttempt, listUnitTestAttempts } from "@/lib/store";
...
export async function GET(_request: NextRequest, { params }: Params) {
  ...
  const attempts = await listUnitTestAttempts(id);
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const attempt = await addUnitTestAttempt(id, score, total, termResults);
